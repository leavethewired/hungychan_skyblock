...
vec3 applyCRT(vec2 uv)
{
    ...
    downsampledColor.r = texture2D(colortex0, redUV).r;
	downsampledColor.g = texture2D(colortex0, uv).g;
    downsampledColor.b = texture2D(colortex0, blueUV).b;

	downsampledColor = pow(downsampledColor, vec3(0.45f));
